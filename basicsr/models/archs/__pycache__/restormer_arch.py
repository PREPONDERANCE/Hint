...
class MDMLP(nn.Module):
    def __init__(self, dim, height, width, bias, LayerNorm_type):
        ...
        self.dwconv_c = nn.Conv2d(
            dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=bias
        )
        self.dwconv_h = nn.Conv2d(
            height, height, kernel_size=3, stride=1, padding=1, groups=height, bias=bias
        )
        self.dwconv_w = nn.Conv2d(
            width, width, kernel_size=3, stride=1, padding=1, groups=width, bias=bias
        )
...
class PMFLFFN(nn.Module):
    def __init__(
        ...
        self.dwconv_5 = nn.Conv2d(
            dim, dim, kernel_size=5, stride=1, padding=2, groups=dim, bias=bias
        )
        self.dwconv_7 = nn.Conv2d(
            dim, dim, kernel_size=7, stride=1, padding=3, groups=dim, bias=bias
        )
        # self.norm_c = LayerNorm(dim, LayerNorm_type)
        # self.norm_h = LayerNorm(height, LayerNorm_type)
        # self.norm_w = LayerNorm(width, LayerNorm_type)
...
class MPartial_conv3InFFN(nn.Module):
    def __init__(self, dim, n_div=4):
        ...
        self.partial_conv5 = nn.Conv2d(
            self.dim_conv, self.dim_conv, 5, 1, 2, bias=False
        )
        self.partial_conv7 = nn.Conv2d(
            self.dim_conv, self.dim_conv, 7, 1, 3, bias=False
        )
    ...
    def forward(self, x):
        ...
        x1, x2, x3, x4 = torch.split(
            x, [self.dim_conv, self.dim_conv, self.dim_conv, self.dim_untouched], dim=1
        )
        x1 = self.partial_conv3(x1)
...
class TransformerBlock_v00(nn.Module):
    # cancel the token-mixer part
    def __init__(
        self, h, w, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type
    ):
