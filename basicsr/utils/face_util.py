...
try:
    ...
    print(
        "Please install dlib before testing face restoration."
            ...
            if len(det_face) == 0:
                ...
                if len(det_face) > 1:
                    ...
                    for i in range(len(det_face)):
                        face_area = (
                            det_face[i].rect.right() - det_face[i].rect.left()
    ...
    def warp_crop_faces(self, save_cropped_path=None, save_inverse_affine_path=None):
        ...
        for idx, landmark in enumerate(self.all_landmarks_5):
            ...
            self.similarity_trans.estimate(
                self.face_template, landmark * self.upscale_factor
            )
    ...
    def paste_faces_to_input_image(self, save_path):
        ...
        for restored_face, inverse_affine in zip(
            ...
            inv_mask_erosion = cv2.erode(
                inv_mask,
                np.ones((2 * self.upscale_factor, 2 * self.upscale_factor), np.uint8),
            )
            ...
            upsample_img = (
                inv_soft_mask * inv_restored_remove_border
